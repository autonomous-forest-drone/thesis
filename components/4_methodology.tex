...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{airsim_env.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{texture_pack.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{lighting_variation.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{goal_view.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{spawn_clearance.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{stage_views.png}
...
The drone sees the world through a single forward-facing camera capturing 128 $\times$ 128 pixel images at 30\,Hz with a 62\textdegree{} field of view, the same resolution, frame rate, and angle as the Arducam IMX219 mounted on the physical drone, ensuring no sensor mismatch between simulation and reality. Matching simulation and physical sensor parameters is a standard practice for minimising the reality gap \cite{Zhao2020,Salvato2021,Loquercio2019}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{grayscale.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\textwidth]{clahe.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\textwidth]{gaussian_noise.png}
...
\input{components/tables/table2_obs_action.tex}

\subsubsection{Training in a simulation --- Reward system}

The reward function implementation is as follows. Table \ref{tab:reward} lists every component with its value.

The progress term awards $+$25 per metre of forward progress toward the goal.

The time penalty applies $-$0.2 per step.

A soft lateral offset penalty is applied proportionally when the drone drifts more than 0.5\,m perpendicular to the spawn-to-goal axis.
...
\input{components/tables/table3_reward.tex}

\subsubsection{Training in a simulation --- The model network}

The actor uses a dual-stream network as described in the Methods/Approach section. The implementation details are as follows.

The visual stream applies a CNN to the 8-frame grayscale stack (128$\times$128 per frame), producing a compact visual feature vector.

The state stream applies a small fully connected network to the 4-element scalar vector.

Both outputs are concatenated and fed into policy and value heads trained end-to-end using PPO \cite{Schulman2017}.

The critic uses two fully connected layers of 128 units each. Specific layer sizes and all hyperparameters are listed in Table \ref{tab:hyperparameters}.
...
\input{components/tables/table4_hyperparameters.tex}

\subsubsection{Testing in a simulation --- Evaluation protocol}
...
The best policy checkpoint was evaluated over 50 episodes per configuration. Conditions are defined by the number of trees placed in the 15\,$\times$\,3\,m corridor: sparse (2 trees) and dense (3 trees), combined with a fixed speed within range of 0\,m/s as a minimum and 3\,m/s as a maximum. An episode was counted as successful if and only if the drone reached the goal waypoint without any collision and without leaving the defined corridor ($\pm$3\,m lateral hard kill) \cite{Kabas2022}. The goal distance for the simulation testing corridor, like in training, is 15\,m in length. The testing data structure is shown in Table \ref{tab:data_structure}.

\input{components/tables/table5_data_structure.tex}

\subsection{Real-World}

\subsubsection{Preparation --- The hardware setup}

The physical drone used for real-world evaluation was a \textit{Holybro S500 V2} quadcopter (500\,mm wheelbase) with a total all-up weight of approximately 1.75\,kg. The flight controller was a Pixhawk 6C running PX4 firmware v1.14+, operating in altitude control mode. The companion computer was an NVIDIA Jetson Orin Nano (8\,GB), selected for its ability to run TensorRT-optimised inference in real time \cite{Kabas2022}. The camera was an Arducam IMX219 configured to match the simulation sensor parameters exactly: 128 $\times$ 128 pixels, 30\,fps, 62\textdegree{} FOV, following the sensor-matching approach recommended for sim-to-real transfer \cite{Zhao2020,Salvato2021}. Figure \ref{fig:drone_hardware} shows how the setup looks in real life and Table \ref{tab:hardware} lists the key components of the setup.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{drone_hardware.png}
    \caption{The Holybro S500 V2 quadrotor used for real-world evaluation, equipped with the Arducam IMX219 camera, Pixhawk 6C flight controller, and NVIDIA Jetson Orin Nano companion computer.}
...
Robot Operating System 2 (ROS2) connects all onboard hardware. The camera streams frames to the Jetson over a ROS2 topic, where OpenCV applies CLAHE and Gaussian noise preprocessing before passing frames to the inference node.

At the same time, ROS2 receives Inertial Measurement Unit (IMU), Global Positioning System (GPS), and velocity data from the Pixhawk over serial communication.

The inference node combines both streams into the 8-frame image stack and 4D state vector, runs the PPO policy \cite{Schulman2017}, and publishes velocity commands back to the Pixhawk via Micro Air Vehicle Link (MAVLink) using MAVSDK-Python in altitude flight mode.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{ros2_pipeline.png}
...
The trained PPO policy was converted from PyTorch to TensorRT (NVIDIA's deep learning inference optimiser) before deployment, bringing inference latency down to 20--30\,ms on the Jetson, fast enough for the 30\,Hz control loop. Velocity commands from the inference node are sent to the Pixhawk via MAVLink using MAVSDK-Python, which executes them in altitude control mode. A QGroundControl ground station on a laptop handled telemetry monitoring and safety override during all flights. Table \ref{tab:software} summarises the full software stack.
...
The same 50 flights per condition were conducted as in simulation, across identical configurations: sparse (2 trees in corridor) and dense (3 trees in corridor). An obstacle was counted as avoided if the tree entered the 6\,m detection corridor, no physical contact occurred, and the drone maintained forward progress. Collision detection was determined by an accelerometer spike on the Pixhawk IMU exceeding a predefined threshold. Any flight requiring manual RC failsafe activation was recorded as a failure \cite{Azzam2024}. Table \ref{tab:data_structure} retains the same data structure for real-world trials as for simulation.
