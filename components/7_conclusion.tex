...
Taken as a whole, the experimental results are encouraging. The simulated drone reached an episode-level success rate of 89.00\,\% (and a per-obstacle success rate of 95.60\,\%) across the two density conditions, and that performance carried over to the real forest with an episode-level transfer efficiency of 80.90\,\% and a per-obstacle transfer efficiency of 83.68\,\%. Neither figure is 100\,\%, which means the system is clearly not production-ready yet, but the gap between the simulated and real-world numbers is small enough -- and the absolute numbers are high enough across 200 evaluation episodes -- that the results are statistically meaningful rather than noise.
