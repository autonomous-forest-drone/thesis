...
After training, the policy is evaluated under fixed, deterministic conditions: noise and randomisation are disabled, and the curriculum is fixed. \textbf{50 episodes are run in the sparse stage and 50 episodes in the dense stage}, giving 100 simulation evaluation episodes in total. An episode is counted as successful if the drone reaches the end of the corridor without colliding with any tree and without leaving the corridor laterally.
...
Real-world evaluation mirrors the simulation evaluation. \textbf{50 flights per stage} are conducted in a Swedish forest of naked tree trunks, with each flight set up in a different physical corridor so that no two episodes share exactly the same tree configuration. The two stages -- sparse and dense -- are matched as closely as possible to their simulation counterparts in terms of the average inter-trunk spacing measured before each session.
