...
Figure \ref{fig:tensorboard_placeholder} shows the PPO training curves recorded in TensorBoard over the full training run. The reward curve demonstrates the characteristic steady improvement and convergence behaviour of PPO \cite{Schulman2017}, with no catastrophic policy collapse observed across the three curriculum stages. Episode length stabilises as the agent learns to reach the goal efficiently rather than timing out.

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textit{[TensorBoard training graphs will be inserted here.}\\
    \textit{Graphs to include: mean episode reward over timesteps,}\\
    \textit{episode length mean, policy loss, value loss, entropy.]}
    \vspace{2cm}}}
    \caption{PPO training curves from TensorBoard showing reward, episode length, and loss metrics over the full training run.}
    \label{fig:tensorboard_placeholder}
\end{figure}
